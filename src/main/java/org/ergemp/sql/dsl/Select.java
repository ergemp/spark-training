...
public class Select {
    public static void main(String[] args) {
        ...
        df.select(col("customer_id"),col("gender")).show();
        df.select("customer_id","gender").show();

        //select all column
        df.select("*").show();

        //select and groupBy and agg and count

        df.select(col("gender")).groupBy("gender").agg(count("gender").alias("total")).show();
