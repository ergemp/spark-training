    ...
    public static void main(String[] args) {
        ...
        StructType schema2 = DataTypes.createStructType(
                new StructField[]{
                        ...
                        createStructField("props", createStructType(new StructField[]{
                                createStructField("hair",StringType, true),
                                createStructField("eye",StringType, true),
                        }), false)
                });

        Dataset<Row> df = spark.read().schema(schema2).json("data/mapType.json");
        df.printSchema();
        df.show();
